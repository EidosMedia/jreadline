...
public class KeyMapper {
    ...
    public static KeyOperation mapQuoteKeys(String keys, String operation) {
        return new KeyOperation(mapKeys(quotePattern.split(keys)[1]), OperationMapper.mapToFunction(operation));
    }
    ...
    private static int[] mapRandomKeys(String randomKeys, boolean control, boolean meta) {
        if (randomKeys == null)
            throw new RuntimeException("ERROR JReadline didnt find any keys after meta/control: " + randomKeys
                    + " Check your inputrc.");
