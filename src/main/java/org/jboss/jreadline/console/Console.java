...
public class Console {
    ...
    public void reset(Settings settings) throws IOException {
        ...
        if (settings.isHistoryPersistent())
            history = new FileHistory(settings.getHistoryFile().getAbsolutePath(), settings.getHistorySize());
        else
    ...
    private void setTerminal(Terminal term, InputStream in, OutputStream stdOut, OutputStream stdErr, 
            boolean uiEnabled) {
        terminal = term;
        terminal.init(in, stdOut, stdErr, uiEnabled);
    }
    ...
    public void pushToStdOut(String input) throws IOException {
        if (input != null && input.length() > 0) {
            ...
            if (currentOperation != null && ControlOperator.isRedirectionOut(currentOperation.getControlOperator()))
    ...
    public void pushToStdErr(String input) throws IOException {
        if (input != null && input.length() > 0) {
            if (currentOperation != null && ControlOperator.isRedirectionErr(currentOperation.getControlOperator()))
            ...
            if (currentRow > -1) {
                ...
                if (currentRow + (newLine.length() / getTerminalWidth()) - cursorRow >= getTerminalHeight()) {
                    int numNewRows = currentRow
                            + ((newLine.length() + buffer.getPrompt().length()) / getTerminalWidth()) - cursorRow
                            - getTerminalHeight();
    ...
    private void writeChar(int c, Character mask) throws IOException {
        ...
        if (buffer.getCursorWithPrompt() > getTerminalWidth() && buffer.getCursorWithPrompt() % getTerminalWidth() == 1) {
        ...
        if (buffer.getCursor() < buffer.length()) {
            ...
            if (buffer.totalLength() > getTerminalWidth() && (buffer.totalLength() - 1) % getTerminalWidth() == 1) {
    ...
    private boolean performAction(EditAction action) throws IOException {
        ...
        if (action.getAction() == Action.MOVE) {
            ...
            //System.out.println("action start: " + action.getStart());
            //System.out.println("action end: " + action.getEnd());
    ...
    public final void moveCursor(final int where) throws IOException {
        if (editMode.getMode() == Mode.VI
                && (editMode.getCurrentAction() == Action.MOVE || editMode.getCurrentAction() == Action.DELETE)) {
    ...
    private void drawLine(String line) throws IOException {
        ...
        if (line.length() > getTerminalWidth() || (line.length() + Math.abs(buffer.getDelta()) > getTerminalWidth())) {
    ...
    */
    ...
    private void complete() throws IOException {
        ...
        if (ControlOperatorParser.doStringContainPipeline(buffer.getLine())) {
            pipeLinePos = ControlOperatorParser.findLastPipelinePositionBeforeCursor(buffer.getLine(),
                    buffer.getCursor());
        ...
        for (Completion completion : completionList) {
            ...
            if (pipeLinePos > 0) {
                co = findAliases(buffer.getLine().substring(pipeLinePos, buffer.getCursor()), buffer.getCursor()
                        - pipeLinePos);
            } else {
        ...
        else if (possibleCompletions.size() == 1 && possibleCompletions.get(0).getCompletionCandidates().size() == 1) {
            // some formatted completions might not be valid and shouldnt be displayed
            displayCompletion(possibleCompletions.get(0).getCompletionCandidates().get(0), possibleCompletions.get(0)
                    .getFormattedCompletionCandidates().get(0), true);
        }
        // more than one hit...
        else {
            ...
            // check size
                ...
                if (completions.size() > 100) {
                    if (displayCompletion) {
                        ...
                        terminal.writeToStdOut(Config.getLineSeparator() + "Display all " + completions.size()
    ...
    private void syncCursor() throws IOException {
        if (buffer.getCursor() != buffer.getLine().length())
            terminal.writeToStdOut(Buffer.printAnsi((Math.abs(buffer.getCursor() - buffer.getLine().length()) + "D")));
    ...
    private ConsoleOutput parseOperations() throws IOException {
        ...
        if (op.getControlOperator() == ControlOperator.OVERWRITE_OUT
            ...
            if (operations.size() == 0) {
                ...
                currentOperation = op;
                output = new ConsoleOutput(op, redirectPipeOutBuffer.toString(), redirectPipeErrBuffer.toString());
            }
            ...
            if (operations.size() > 0) {
                ...
                if (nextOperation.getBuffer().length() > 0) {
                    ...
                    try {
                        output = new ConsoleOutput(new ConsoleOperation(nextOperation.getControlOperator(),
                                op.getBuffer()), FileUtils.readFile(new File(Parser
                                .switchEscapedSpacesToSpacesInWord(files.get(0)))), redirectPipeErrBuffer.toString());
                    }
                    ...
                    pushToStdErr(settings.getName() + ": syntax error near unexpected token '<'"
                            + Config.getLineSeparator());
                    currentOperation = null;
                    output = new ConsoleOutput(new ConsoleOperation(ControlOperator.NONE, ""));
                }
            } else {
                pushToStdErr(settings.getName() + ": syntax error near unexpected token 'newline'"
                        + Config.getLineSeparator());
                currentOperation = null;
                output = new ConsoleOutput(new ConsoleOperation(ControlOperator.NONE, ""));
            }
    ...
    private ConsoleOutput processInternalCommands(ConsoleOutput output) throws IOException {
        if (output.getBuffer() != null) {
            if (settings.isAliasEnabled() && output.getBuffer().startsWith(InternalCommands.ALIAS.getCommand())) {
    ...
    private ConsoleOutput findAliases(ConsoleOutput operation) {
        if (settings.isAliasEnabled()) {
            ...
            if (alias != null) {
                operation.setConsoleOperation(new ConsoleOperation(operation.getControlOperator(), alias.getValue()
                        + operation.getBuffer().substring(command.length())));
            }
        }
        return operation;
    }
    ...
    private CompleteOperation findAliases(String buffer, int cursor) {
        if (settings.isAliasEnabled()) {
            ...
            Alias alias = aliasManager.getAlias(command);
            if (alias != null) {
                return new CompleteOperation(alias.getValue() + buffer.substring(command.length()), cursor
                        + (alias.getValue().length() - command.length()));
            }
        }
    ...
    private void persistRedirection(String fileName, ControlOperator redirection) throws IOException {
        ...
        try {
            if (redirection == ControlOperator.OVERWRITE_OUT)
                FileUtils.saveFile(new File(Parser.switchEscapedSpacesToSpacesInWord(fileName)),
                        redirectPipeOutBuffer.toString(), false);
